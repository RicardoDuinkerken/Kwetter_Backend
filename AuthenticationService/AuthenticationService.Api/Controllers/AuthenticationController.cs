...
{   
    ...
    public class AuthenticationController : ControllerBase
    {
        ...
        public async Task<IActionResult> Register([FromBody] CreateAccountRequestDto request)
        {
            ...
            try
            {
                return Ok(_authService.Register(request.Email, request.Password));
            }
            catch (Exception e)
            {
                _logger.LogError("{E}", e);
        ...
        [ProducesResponseType(typeof(string), StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> ChangePassword([FromBody] CreateAccountRequestDto request)
        {
            _logger.LogInformation("Login Invoked for email: {E}", request.Email);
            try
            {
        ...
        [ProducesResponseType(typeof(string), StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> Login([FromBody] CreateAccountRequestDto request)
        {
            _logger.LogInformation("Login Invoked for email: {E}", request.Email);
            try
            {
